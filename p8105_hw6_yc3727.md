...
pregnancy, and mother’s age at delivery) because I thought health
conditions and characteristics of mothers before and during pregnancy
may directly affect the baby’s birthweight.
...
values compared to the main effects and interaction models. Therefore,
it appears that the interaction model is the best fit for this dataset
because lower RMSE values indicate better fit.
...
```
    ...
    ## date created (size, mb): 2020-11-26 15:11:37 (0.008)

    ## file min/max dates: Inf / -Inf

    ## Error: Problem with `mutate()` input `name`.
    ## x no applicable method for 'recode' applied to an object of class "function"
    ## i Input `name` is `recode(id, USW00094728 = "CentralPark_NY")`.

``` r
head(weather_df)
```

    ## Error in head(weather_df): object 'weather_df' not found
